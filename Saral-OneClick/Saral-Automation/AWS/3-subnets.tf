...
resource "aws_subnet" "docdb-private-subnet-a" {
  vpc_id            = aws_vpc.main.id
  cidr_block        = "10.0.128.0/19"
  availability_zone = data.aws_availability_zones.available.names[0]  # Assign the fifth AZ from the available list
...
resource "aws_subnet" "docdb-private-subnet-b" {
  vpc_id            = aws_vpc.main.id
  cidr_block        = "10.0.160.0/19"
  availability_zone = data.aws_availability_zones.available.names[1]  # Assign the sixth AZ from the available list
